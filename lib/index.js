...
function promise_obj(obj) {
    ...
    const [ keys, promises ] = [ Object.keys(promises_map), Object.values(promises_map) ];
    
    return Promise.all(promises)
    .then(result_arr => {
        for (let idx = 0; idx < result_arr.length; idx++) {
            const abs_key = keys[idx];
            const result = result_arr[idx];

            let obj_ref = obj;
            const splitted_keys = abs_key.split('.');
            for (let j = 0; j < splitted_keys.length - 1; j++)
                obj_ref = obj_ref[splitted_keys[j]];

            obj_ref[splitted_keys[splitted_keys.length - 1]] = result;
        }
